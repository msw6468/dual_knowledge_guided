...
\textbf{Center-wise analysis.} When examining the center-specific recall scores ~\ref{fig:2_center_wise_analysis}, we observed that performance generally rose across all hospitals, in line with the overall performance improvement. Notably, SSV achieved a substantial gain when Mixup was incorporated, suggesting that Mixup's data augmentation, by increasing the number of samples in the minority class, was a contributing factor.
\begin{figure}[!ht]
    \centering
